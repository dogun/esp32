...
ssize_t recv_data(data_pack* dp) {
	check_recv();
	HAL_StatusTypeDef ret = HAL_UART_Receive(&huart3, (uint8_t*) dp, RECV_HEAD_LEN, 500);
	if (ret != HAL_OK) {
		return -1;
	}
	if (dp->len > MAX_LEN) {
	...
	ret = HAL_UART_Receive(&huart3, (uint8_t*) dp + RECV_HEAD_LEN, dp->len, 1000);
	if (ret != HAL_OK) {
		return -3;
	}
	return 0;
  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1) {
		if (SERVER_ADDR_H != ME_ADDR_H || SERVER_ADDR_L != ME_ADDR_L) {
			...
			w_pack wp = {0};
			wp.addr_h = SERVER_ADDR_H;
			wp.addr_l = SERVER_ADDR_L;
			wp.channel = SERVER_CHANNEL_NO;
